...
func (m *NetifManager) AddDummyDevice(name string) error {
	...
	ip, err := ParseMAC(prefix, l.Attrs().HardwareAddr)
	if err != nil {
		return err
	}
	err = m.AddrAdd(l, &netlink.Addr{IPNet: netlink.NewIPNet(ip)})
	if err != nil {
		return err
	}
	return err
...
func (m *NetifManager) GetDummyDeviceIPv6(name string) ([]net.IP, error) {
	allAddresses := []net.IP{}
	...
	addrs, err := m.AddrList(link, netlink.FAMILY_V6)
	if err != nil {
		return allAddresses, err
	}
	for _, a := range addrs {
		allAddresses = append(allAddresses, a.IP)
	}
